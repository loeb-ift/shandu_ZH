...
1. **初始化**：研究過程從用戶的查詢和初步參數開始。
   ...
   - 設置初始狀態：記錄開始時間、設置狀態為「初始化中」，並記錄當前日期
   - 生成研究計劃：使用 LLM 生成詳細的研究計劃
   - 解析研究計劃：使用正則表達式從 LLM 回應中提取結構化信息
   - 設置默認值：如果某些部分未能從 LLM 回應中提取，則設置默認值
   - 格式化研究計劃：將提取的信息格式化為 Markdown 格式
   - 更新狀態：將研究計劃添加到狀態中，為後續節點準備
   ...
   初始化節點是研究工作流程的入口點，在 LangGraph 狀態圖中設置為第一個執行的節點。初始化完成後，工作流程會進入查詢生成節點 (Generate Queries)，開始實際的研究過程。
